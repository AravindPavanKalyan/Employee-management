...
export class EmployeeService {
  ...
  createEmployee(formData: any) {
    ...
    return this.http
      .post(environment.employeeRestApi, formData)
      .pipe(
        map((res: any) => {
  ...
  //list employees
  getEmployees(): Observable<IEmployee[]> { // 1.get the req from comp
    // 2. send the form data to the REST API
    //2.1 what's the REST API URL? https://jsonplaceholder.typicode.com/users
    // 2.2 what's the Http Method? POST
    // 2.3 what's the REST API Client Tool? HttpClient
    return this.http.get<IEmployee[]>(environment.employeeRestApi)
      .pipe(map((res:IEmployee[]) => { // 3. get the res
        console.log(res);
  ...
  deleteEmployee(empId: any) {
    ...
    // 2.3 what's the REST API Client Tool? HttpClient
    return this.http
      .delete(`${environment.employeeRestApi}/${empId}`)
      .pipe(
        map((res: any) => {
          // 3. get theres from the REST API
          console.log(res);
